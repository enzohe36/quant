...
Continuous position sizing in [-1, 1] with Gaussian + tanh policy.
...
class PolicyNetwork(nn.Module):
    def __init__(self, stock_groups, market_groups, stock_feature_indices,
        ...
        position_dim = 16
        self.position_proj = nn.Linear(1, position_dim)
    ...
    def forward(self, x, position):
        ...
        action_mean = self.policy_head(pooled).squeeze(-1)
        return action_mean, self.log_std, self.value_head(pooled).squeeze(-1)


def _tanh_log_prob(raw_action, mean, log_std, eps):
    ...
    tanh_correction = torch.log(1.0 - torch.tanh(raw_action).pow(2) + eps)
    return normal_log_prob - tanh_correction


def sample_stochastic(model, states, position):
    action_mean, log_std, values = model(states, position)
    std = log_std.exp()
    noise = torch.randn_like(action_mean)
    raw_action = action_mean + std * noise
    action = torch.tanh(raw_action)
    log_prob = _tanh_log_prob(raw_action, action_mean, log_std, 1e-6)
    return action, raw_action, log_prob, values


def select_greedy(model, states, position):
    action_mean, _, values = model(states, position)
    return torch.tanh(action_mean), values


def evaluate_actions(model, states, raw_actions, position):
    action_mean, log_std, values = model(states, position)
    log_prob = _tanh_log_prob(raw_actions, action_mean, log_std, 1e-6)
    # Gaussian entropy (analytic, without tanh correction — standard approximation)
    entropy = 0.5 * math.log(2 * math.pi * math.e) + log_std
    # Broadcast scalar entropy to batch
    entropy = entropy.expand(values.shape)
...
class RolloutBuffer:
    ...
    def __init__(self, n_episodes, episode_length, lookback, global_market_features):
        ...
        self.actions = np.empty((n_episodes, episode_length), dtype=np.float32)
    ...
    for batch_start in range(0, n_padded_transitions, cfg.batch_size):
        ...
        with torch.no_grad():
            ...
            if valid_mask is not None:
                ...
                nv = bw.sum().clamp(min=1)
                ret_mean = (batch_returns * bw).sum() / nv
                explained_var_num += ((residual ** 2) * bw).sum().item()
                explained_var_den += (((batch_returns - ret_mean) ** 2) * bw).sum().item()
            else:
                ret_mean = batch_returns.mean()
                explained_var_num += (residual ** 2).sum().item()
                explained_var_den += ((batch_returns - ret_mean) ** 2).sum().item()
...
def plot_training(history, path, cfg):
    ...
    Row 1: policy loss, value loss, gradient norm
    Row 2: clip fraction, approx KL, explained variance
    Row 3: returns (with ablation), beat rate, val max drawdown
    Row 4: mean turnover, log_std, position heatmap
    """
    ...
    axes[0, 2].plot(epochs, history["grad_norm"], color=C_TRAIN, alpha=0.8)
    axes[0, 2].set_ylabel("Norm")
    axes[0, 2].set_title("Gradient Norm")
    axes[0, 2].grid(True, alpha=0.3)

    # Row 2: PPO diagnostics (cont)
    ...
    axes[1, 2].set_ylabel("Explained Var")
    axes[1, 2].set_title("Explained Var")
    axes[1, 2].axhline(y=0, color="gray", linestyle="--", alpha=0.5)
    axes[1, 2].grid(True, alpha=0.3)

    # Row 3: Performance
    ablation_epochs = history.get("ablation_epochs", [])
    axes[2, 0].plot(epochs, history["train_model"], color=C_TRAIN, label="Train model", alpha=0.8)
    axes[2, 0].plot(epochs, history["val_model"], color=C_VAL, label="Val model", alpha=0.8)
    axes[2, 0].plot(epochs, history["train_baseline"], color=C_TRAIN, linestyle="--", label="Train baseline", alpha=0.5)
    axes[2, 0].plot(epochs, history["val_baseline"], color=C_VAL, linestyle="--", label="Val baseline", alpha=0.5)
    if ablation_epochs:
        axes[2, 0].plot(
    ...
    axes[2, 0].set_title("Return")
    axes[2, 0].legend(fontsize=7)
    axes[2, 0].grid(True, alpha=0.3)

    axes[2, 1].plot(epochs, history["beat_rate"], color=C_VAL, alpha=0.8)
    axes[2, 1].set_xlabel("Epoch")
    axes[2, 1].set_ylabel("Beat Rate")
    axes[2, 1].set_title("Beat Rate")
    axes[2, 1].set_ylim(0, 1)
    axes[2, 1].grid(True, alpha=0.3)

    axes[2, 2].plot(epochs, history["val_max_drawdown"], color=C_VAL, alpha=0.8)
    axes[2, 2].set_xlabel("Epoch")
    axes[2, 2].set_ylabel("Drawdown")
    axes[2, 2].set_title("Val Max Drawdown")
    axes[2, 2].set_ylim(bottom=0)
    axes[2, 2].grid(True, alpha=0.3)

    # Row 4: Policy behavior
    axes[3, 0].plot(epochs, history["mean_turnover"], color=C_VAL, alpha=0.8)
    axes[3, 0].set_xlabel("Epoch")
    axes[3, 0].set_ylabel("Turnover")
    axes[3, 0].set_title("Mean Turnover")
    axes[3, 0].set_ylim(bottom=0)
    axes[3, 0].grid(True, alpha=0.3)

    axes[3, 1].plot(epochs, history["log_std"], color=C_TRAIN, alpha=0.8)
    axes[3, 1].set_xlabel("Epoch")
    axes[3, 1].set_ylabel("log_std")
    axes[3, 1].set_title("Log Std")
    ...
    if pos_hists:
        ...
        fig.colorbar(im, ax=axes[3, 2], label="Frequency")
    axes[3, 2].set_xlabel("Epoch")
    axes[3, 2].set_ylabel("Position")
    axes[3, 2].set_title("Position Distribution")
...
def train(cfg):
    ...
    best_smoothed_return = -float("inf")
    patience_counter = 0
    start_epoch = 1
    ...
    if os.path.exists(latest_path):
        ...
        if val_series:
            ...
            best_smoothed_return = max(
                float(np.mean(val_series[max(0, i + 1 - window) : i + 1]))
                for i in range(len(val_series))
            )
        else:
            best_smoothed_return = ckpt.get("best_smoothed_return", -float("inf"))

        _log()
        _log(
            f"Resumed from epoch {ckpt['epoch']}"
            f" (best smoothed {best_smoothed_return:.4f},"
    ...
    def _build_checkpoint():
        return {
            ...
            "epoch": epoch,
            "best_smoothed_return": best_smoothed_return,
    ...
    for epoch in range(start_epoch, cfg.n_epochs + 1):
        ...
        if epoch == start_epoch:
            ...
            _log(f"    {'Val episodes':<20s}: {len(val_episodes)} (epoch {epoch})")
        ...
        for key, value in [
            ("train_model", model_return),
            ("train_baseline", baseline_return),
            ("val_model", val_aggregated["model_return"]),
            ...
            ("mean_turnover", val_aggregated["mean_turnover"]),
        ...
        smoothed_val_return = float(np.mean(recent_vals))

        improved = smoothed_val_return > best_smoothed_return
        if improved:
            best_smoothed_return = smoothed_val_return
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        _log(f"    {'Val smoothed':<20s}: {smoothed_val_return:.4f}")
        _log(f"    {'Beat rate':<20s}: {val_aggregated['beat_rate']:.4f}")
        _log(f"    {'Val max drawdown':<20s}: {val_aggregated['max_drawdown']:.4f}")
        _log(f"    {'Mean turnover':<20s}: {val_aggregated['mean_turnover']:.4f}")
        ...
        _log(f"    {'Explained var':<20s}: {epoch_losses['explained_variance']:.4f}")
        ...
        if improved:
            _log(f"    {'New best smoothed':<20s}: {best_smoothed_return:.4f}")
    ...
    _log()
    _log(f"Training complete, best smoothed validation: {best_smoothed_return:.4f}")
    ...
    _log(f"    {'Model return':<20s}: {test_aggregated['model_return']:.4f}")
    _log(f"    {'Baseline return':<20s}: {test_aggregated['baseline_return']:.4f}")
    _log(f"    {'Beat rate':<20s}: {test_aggregated['beat_rate']:.4f}")
    _log(f"    {'Max drawdown':<20s}: {test_aggregated['max_drawdown']:.4f}")
    _log(f"    {'Mean turnover':<20s}: {test_aggregated['mean_turnover']:.4f}")
