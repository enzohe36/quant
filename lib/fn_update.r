update <- function(
    ...
    if (
      !all(
        select(data[nrow(data), ], is.numeric) ==
          select(data_update_i, is.numeric) %in% TRUE
      )
    ) {
      ifelse(
  ...
  latest <- rbindlist(
    lapply(
      ...
      function(df) {
        ...
        out <- df[nrow(df), ] %>%
          mutate(
            name = data_name$name[data_name$symbol == symbol],
            .after = symbol
          ) %>%
          cbind(fundflow[fundflow$symbol == symbol, -1])
        return(out)
      }
    )
  ) %>%
  ...
  portfolio_eval <- lapply(
    ...
    function(df) {
      ...
      if (!any(data$date == df$buy)) stop(
        glue("Purchase date of {symbol} does not exist")
      )
      ...
      out <- list(
        buy = df$buy,
        symbol = symbol,
        name = data_name[data_name$symbol == symbol, "name"],
        cost = df$cost,
        r = r,
        action = ifelse(
          r >= r_max | r <= r_min | last(data$date) - df$buy >= t_max,
          "SELL",
          "HOLD"
        )
      )
    ...
    mutate(buy = as.character(as_date(buy))) %>%
    mutate(across(where(is.numeric), round, 3)) %>%
    mutate(across(where(is.numeric), format, nsmall = 2)) %>%
